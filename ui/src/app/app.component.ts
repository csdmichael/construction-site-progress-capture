    ...
    <ion-app>
      ...
      <ion-content class="ion-padding">
        ...
        <ng-template #detail>
          ...
          <ng-template #empty><p>Select a capture from the list.</p></ng-template>
        </ng-template>
      </ion-content>
    </ion-app>
  `,
...
export class AppComponent {
  readonly title = "Construction Site Progress Capture";
  readonly screens = ["1", "2", "3", "4", "Work Package List", "Interaction Details"];
  readonly screen = signal("1");
  readonly items = signal<Capture[]>([]);
  readonly selected = signal<Capture | null>(null);
  readonly error = signal('');
  private readonly http = inject(HttpClient);

  constructor() {
    this.http.get<Capture[]>(apiUrl('/api/captures')).subscribe({
      next: (items) => this.items.set(items),
      error: () => this.error.set('Could not reach the API.'),
    });
  }

  select(item: Capture): void {
